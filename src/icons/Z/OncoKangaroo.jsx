import React from 'react';

export const iconData = {
  "id": "OncoKangaroo",
  "name": "OncoKangaroo",
  "category": "Z",
  "nodes": [
    [
      "path",
      {
        "d": "M 9.78 12.00 a 2.22 2.22 0 1 0 4.44 0 a 2.22 2.22 0 1 0 -4.44 0"
      }
    ],
    [
      "path",
      {
        "d": "M 7.79 12.00 a 4.21 4.21 0 1 0 8.41 0 a 4.21 4.21 0 1 0 -8.41 0"
      }
    ],
    [
      "path",
      {
        "d": "M 5.94 12.00 a 6.06 6.06 0 1 0 12.12 0 a 6.06 6.06 0 1 0 -12.12 0"
      }
    ],
    [
      "path",
      {
        "d": "M 3.70 12.00 a 8.30 8.30 0 1 0 16.59 0 a 8.30 8.30 0 1 0 -16.59 0"
      }
    ]
  ]
};

export const OncoKangaroo = React.forwardRef(({ size = 24, className = "", color = "currentColor", strokeWidth = 2, children, ...rest }, ref) => {
  return (
    <svg
      ref={ref}
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      stroke={color}
      strokeWidth={strokeWidth}
      strokeLinecap="round"
      strokeLinejoin="round"
      className={className}
      {...rest}
    >
      <path d="M 9.78 12.00 a 2.22 2.22 0 1 0 4.44 0 a 2.22 2.22 0 1 0 -4.44 0" />
      <path d="M 7.79 12.00 a 4.21 4.21 0 1 0 8.41 0 a 4.21 4.21 0 1 0 -8.41 0" />
      <path d="M 5.94 12.00 a 6.06 6.06 0 1 0 12.12 0 a 6.06 6.06 0 1 0 -12.12 0" />
      <path d="M 3.70 12.00 a 8.30 8.30 0 1 0 16.59 0 a 8.30 8.30 0 1 0 -16.59 0" />
      {children}
    </svg>
  );
});

export default OncoKangaroo;
