import React from 'react';

export const iconData = {
  "id": "LavaVertex",
  "name": "LavaVertex",
  "category": "XJ",
  "nodes": [
    [
      "path",
      {
        "d": "M 9.64 12.00 a 2.36 2.36 0 1 0 4.72 0 a 2.36 2.36 0 1 0 -4.72 0",
        "stroke-dasharray": "4 1"
      }
    ],
    [
      "path",
      {
        "d": "M 7.22 12.00 a 4.78 4.78 0 1 0 9.56 0 a 4.78 4.78 0 1 0 -9.56 0"
      }
    ],
    [
      "path",
      {
        "d": "M 4.26 12.00 a 7.74 7.74 0 1 0 15.48 0 a 7.74 7.74 0 1 0 -15.48 0"
      }
    ]
  ]
};

export const LavaVertex = React.forwardRef(({ size = 24, className = "", color = "currentColor", strokeWidth = 2, children, ...rest }, ref) => {
  return (
    <svg
      ref={ref}
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      stroke={color}
      strokeWidth={strokeWidth}
      strokeLinecap="round"
      strokeLinejoin="round"
      className={className}
      {...rest}
    >
      <path d="M 9.64 12.00 a 2.36 2.36 0 1 0 4.72 0 a 2.36 2.36 0 1 0 -4.72 0" stroke-dasharray="4 1" />
      <path d="M 7.22 12.00 a 4.78 4.78 0 1 0 9.56 0 a 4.78 4.78 0 1 0 -9.56 0" />
      <path d="M 4.26 12.00 a 7.74 7.74 0 1 0 15.48 0 a 7.74 7.74 0 1 0 -15.48 0" />
      {children}
    </svg>
  );
});

export default LavaVertex;
