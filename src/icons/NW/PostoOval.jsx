import React from 'react';

export const iconData = {
  "id": "PostoOval",
  "name": "PostoOval",
  "category": "NW",
  "nodes": [
    [
      "path",
      {
        "d": "M 9.87 12.00 a 2.13 2.13 0 1 0 4.27 0 a 2.13 2.13 0 1 0 -4.27 0"
      }
    ],
    [
      "path",
      {
        "d": "M 7.18 12.00 a 4.82 4.82 0 1 0 9.63 0 a 4.82 4.82 0 1 0 -9.63 0"
      }
    ],
    [
      "path",
      {
        "d": "M 4.23 12.00 a 7.77 7.77 0 1 0 15.54 0 a 7.77 7.77 0 1 0 -15.54 0"
      }
    ]
  ]
};

export const PostoOval = React.forwardRef(({ size = 24, className = "", color = "currentColor", strokeWidth = 2, children, ...rest }, ref) => {
  return (
    <svg
      ref={ref}
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      stroke={color}
      strokeWidth={strokeWidth}
      strokeLinecap="round"
      strokeLinejoin="round"
      className={className}
      {...rest}
    >
      <path d="M 9.87 12.00 a 2.13 2.13 0 1 0 4.27 0 a 2.13 2.13 0 1 0 -4.27 0" />
      <path d="M 7.18 12.00 a 4.82 4.82 0 1 0 9.63 0 a 4.82 4.82 0 1 0 -9.63 0" />
      <path d="M 4.23 12.00 a 7.77 7.77 0 1 0 15.54 0 a 7.77 7.77 0 1 0 -15.54 0" />
      {children}
    </svg>
  );
});

export default PostoOval;
