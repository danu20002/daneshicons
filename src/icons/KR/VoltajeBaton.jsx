import React from 'react';

export const iconData = {
  "id": "VoltajeBaton",
  "name": "VoltajeBaton",
  "category": "KR",
  "nodes": [
    [
      "path",
      {
        "d": "M 4.75 12.00 a 7.25 7.25 0 1 0 14.51 0 a 7.25 7.25 0 1 0 -14.51 0"
      }
    ],
    [
      "path",
      {
        "d": "M 6.88 12.00 a 5.12 5.12 0 1 1 10.24 0 a 5.12 5.12 0 1 1 -10.24 0"
      }
    ],
    [
      "path",
      {
        "d": "M 18.75 12.00 L 20.75 12.00"
      }
    ],
    [
      "path",
      {
        "d": "M 16.78 16.78 L 18.19 18.19"
      }
    ],
    [
      "path",
      {
        "d": "M 12.00 18.75 L 12.00 20.75"
      }
    ],
    [
      "path",
      {
        "d": "M 7.22 16.78 L 5.81 18.19"
      }
    ],
    [
      "path",
      {
        "d": "M 5.25 12.00 L 3.25 12.00"
      }
    ],
    [
      "path",
      {
        "d": "M 7.22 7.22 L 5.81 5.81"
      }
    ],
    [
      "path",
      {
        "d": "M 12.00 5.25 L 12.00 3.25"
      }
    ],
    [
      "path",
      {
        "d": "M 16.78 7.22 L 18.19 5.81"
      }
    ]
  ]
};

export const VoltajeBaton = React.forwardRef(({ size = 24, className = "", color = "currentColor", strokeWidth = 2, children, ...rest }, ref) => {
  return (
    <svg
      ref={ref}
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      stroke={color}
      strokeWidth={strokeWidth}
      strokeLinecap="round"
      strokeLinejoin="round"
      className={className}
      {...rest}
    >
      <path d="M 4.75 12.00 a 7.25 7.25 0 1 0 14.51 0 a 7.25 7.25 0 1 0 -14.51 0" />
      <path d="M 6.88 12.00 a 5.12 5.12 0 1 1 10.24 0 a 5.12 5.12 0 1 1 -10.24 0" />
      <path d="M 18.75 12.00 L 20.75 12.00" />
      <path d="M 16.78 16.78 L 18.19 18.19" />
      <path d="M 12.00 18.75 L 12.00 20.75" />
      <path d="M 7.22 16.78 L 5.81 18.19" />
      <path d="M 5.25 12.00 L 3.25 12.00" />
      <path d="M 7.22 7.22 L 5.81 5.81" />
      <path d="M 12.00 5.25 L 12.00 3.25" />
      <path d="M 16.78 7.22 L 18.19 5.81" />
      {children}
    </svg>
  );
});

export default VoltajeBaton;
