import React from 'react';

export const iconData = {
  "id": "StrattoMeter",
  "name": "StrattoMeter",
  "category": "HH",
  "nodes": [
    [
      "path",
      {
        "d": "M 9.96 12.00 a 2.04 2.04 0 1 0 4.08 0 a 2.04 2.04 0 1 0 -4.08 0"
      }
    ],
    [
      "path",
      {
        "d": "M 7.99 12.00 a 4.01 4.01 0 1 0 8.03 0 a 4.01 4.01 0 1 0 -8.03 0"
      }
    ],
    [
      "path",
      {
        "d": "M 6.73 12.00 a 5.27 5.27 0 1 0 10.55 0 a 5.27 5.27 0 1 0 -10.55 0"
      }
    ],
    [
      "path",
      {
        "d": "M 4.97 12.00 a 7.03 7.03 0 1 0 14.06 0 a 7.03 7.03 0 1 0 -14.06 0"
      }
    ],
    [
      "path",
      {
        "d": "M 3.21 12.00 a 8.79 8.79 0 1 0 17.58 0 a 8.79 8.79 0 1 0 -17.58 0"
      }
    ]
  ]
};

export const StrattoMeter = React.forwardRef(({ size = 24, className = "", color = "currentColor", strokeWidth = 2, children, ...rest }, ref) => {
  return (
    <svg
      ref={ref}
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      stroke={color}
      strokeWidth={strokeWidth}
      strokeLinecap="round"
      strokeLinejoin="round"
      className={className}
      {...rest}
    >
      <path d="M 9.96 12.00 a 2.04 2.04 0 1 0 4.08 0 a 2.04 2.04 0 1 0 -4.08 0" />
      <path d="M 7.99 12.00 a 4.01 4.01 0 1 0 8.03 0 a 4.01 4.01 0 1 0 -8.03 0" />
      <path d="M 6.73 12.00 a 5.27 5.27 0 1 0 10.55 0 a 5.27 5.27 0 1 0 -10.55 0" />
      <path d="M 4.97 12.00 a 7.03 7.03 0 1 0 14.06 0 a 7.03 7.03 0 1 0 -14.06 0" />
      <path d="M 3.21 12.00 a 8.79 8.79 0 1 0 17.58 0 a 8.79 8.79 0 1 0 -17.58 0" />
      {children}
    </svg>
  );
});

export default StrattoMeter;
