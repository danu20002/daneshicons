import React from 'react';

export const iconData = {
  "id": "SottoCymbal",
  "name": "SottoCymbal",
  "category": "KY",
  "nodes": [
    [
      "path",
      {
        "d": "M 9.80 12.00 a 2.20 2.20 0 1 0 4.41 0 a 2.20 2.20 0 1 0 -4.41 0",
        "stroke-dasharray": "2 3"
      }
    ],
    [
      "path",
      {
        "d": "M 6.88 12.00 a 5.12 5.12 0 1 0 10.24 0 a 5.12 5.12 0 1 0 -10.24 0"
      }
    ],
    [
      "path",
      {
        "d": "M 4.56 12.00 a 7.44 7.44 0 1 0 14.87 0 a 7.44 7.44 0 1 0 -14.87 0"
      }
    ]
  ]
};

export const SottoCymbal = React.forwardRef(({ size = 24, className = "", color = "currentColor", strokeWidth = 2, children, ...rest }, ref) => {
  return (
    <svg
      ref={ref}
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      stroke={color}
      strokeWidth={strokeWidth}
      strokeLinecap="round"
      strokeLinejoin="round"
      className={className}
      {...rest}
    >
      <path d="M 9.80 12.00 a 2.20 2.20 0 1 0 4.41 0 a 2.20 2.20 0 1 0 -4.41 0" stroke-dasharray="2 3" />
      <path d="M 6.88 12.00 a 5.12 5.12 0 1 0 10.24 0 a 5.12 5.12 0 1 0 -10.24 0" />
      <path d="M 4.56 12.00 a 7.44 7.44 0 1 0 14.87 0 a 7.44 7.44 0 1 0 -14.87 0" />
      {children}
    </svg>
  );
});

export default SottoCymbal;
